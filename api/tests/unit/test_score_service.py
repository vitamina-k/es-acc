...
from esacc.services.score_service import (
    _conn_percentile,
    _fin_percentile,
    compute_exposure,
)
...
async def test_compute_exposure_returns_response() -> None:
    ...
    score_record.__getitem__ = lambda self, key: _score_data[key]
    score_record.get = lambda key, default=None: _score_data.get(key, default)
...
async def test_compute_exposure_aggregated_same_as_data() -> None:
    ...
    # from SAME_AS traversal across multiple data source nodes
    score_record = MagicMock()
    score_record.__getitem__ = lambda self, key: {
        "entity_id": "4:abc:10",
        "entity_labels": ["Person"],
